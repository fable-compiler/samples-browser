...
inheriting from `Inferno.Component` and defining a `render` method, where we can use
...
type [<Pojo>] TodoItemProps =
    ...
    ; editing: bool
    ; onSave: string->unit
    ; onEdit: Browser.Event->unit
    ; onDestroy: Browser.Event->unit
    ; onCancel: Browser.Event->unit
    ; onToggle: Browser.Event->unit }
...
type TodoItem(props) =
    ...
    member this.handleEdit (ev: Browser.MouseEvent) =
        this.props.onEdit(upcast ev)
        this.setState { editText = this.props.todo.title }

    member this.handleKeyDown (e: Browser.KeyboardEvent) =
        match e.which with
        | ESCAPE_KEY ->
            this.setState { editText = this.props.todo.title }
    ...
    member this.handleChange (e: Browser.Event) =
        ...
        R.li [ ClassName className ] [
            R.div [ ClassName "view" ] [
                R.input [
                    ...
                    OnChange (fun e -> this.props.onToggle(e))
                ]
...
type [<Pojo>] TodoFooterProps =
    { count: int
    ; completedCount: int
    ; onClearCompleted: Browser.MouseEvent->unit
    ; nowShowing: string }
...
type TodoApp(props) =
    inherit R.Component<TodoAppProps, TodoAppState>(props)
    ...
    member this.handleChange (ev: Browser.Event) =
        this.setState({ this.state with newTodo = string ev.target?value })

    member this.handleNewTodoKeyDown (ev: Browser.KeyboardEvent) =
    ...
    member this.toggleAll (ev: Browser.Event) =
        this.props.model.toggleAll(not !!ev.target?``checked``)
